 ...
 *
 * <p>This ensures that all cards will have a {@link Rank}, {@link Suit}, and be able to be compared
 * with other cards in the collection.
 *
...
public interface Card extends Comparable<Card> {
  ...
  /**
   * Returns the {@code int} value of the card. This is derived from the {@link Rank}
   *
  ...
  /**
   * Returns {@code true} if this card and the one being compared have the same {@link Rank}.
   *
  ...
  /**
   * Returns {@code true} if this card and the one being compared have the same {@link Suit}.
   *
   * @param card the card used in the comparison
   ...
   *
   * <p>First, the {@link Rank} ordinals are used to compare cards. If they are equal, it then
   * proceeds to compare {@link Suit} ordinals. After which, it will return the difference.
   *
  ...
  default int compareTo(Card card) {
    String nullError = "You must pass a non-null card for comparison";
    Objects.requireNonNull(card, nullError);
    return comparing(Card::getRank).thenComparing(Card::getSuit).compare(this, card);
  }
}
