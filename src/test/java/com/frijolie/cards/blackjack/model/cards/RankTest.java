...
class RankTest {
  ...
  void toString_TenOfHeartsToStringShouldEqual_TH() {
    String message = String.format("Was expecting to receive 'Ten', instead it was: %s", tenOfHearts.toString());
    assertEquals("Ten", tenOfHearts.getRank().toString(), message);
  }

  @Test
  void getValue_TenOfHeartsGetValueShouldReturn_10() {
    String message = String.format("Was expecting the value to equal 10. instead it is: %d", tenOfHearts.getValue());
    assertEquals(10, tenOfHearts.getValue(), message);
  }

  @Test
  void getLetter_TenOfHeartsGetLetterShouldReturn_T() {
    String message = String.format("Was expecting letter to be 'T', instead it is: %s", tenOfHearts.getRank().getLetter());
    assertEquals("T", tenOfHearts.getRank().getLetter(), message);
  }

  @Test
  void getRank_TenOfHeartsGetRankShouldReturn_Rank_TEN() {
    String message = String.format("Was expecting the Rank to equal Rank.TEN. Instead it is: %s", tenOfHearts.getRank());
    assertEquals(Rank.TEN, tenOfHearts.getRank(), message);
  }

  @Test
  void getValue_AllRanksShouldReturnCorrectValue() {
    assertAll("All Ranks should return the correct value",
  ...
  void getLetter_AllRanksShouldReturnCorrectLetter() {
    assertAll("All Ranks should return the correct letter",
