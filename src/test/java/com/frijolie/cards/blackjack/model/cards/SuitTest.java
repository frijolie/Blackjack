...
class SuitTest {
  ...
  void toString_QueenOfSpadesToStringShouldEqual_S() {
    String message = String.format("Queen of Spades suit.toString should return 'S', Instead it is: %s", queenOfSpades.getSuit().toString());
    assertEquals("Spades", queenOfSpades.getSuit().toString(), message);
  }

  @Test
  void getLetter_AllSuitLettersAreCorrect() {
    assertAll("All Suit letters should be correct",
  ...
  void getSymbol_AllSuitSymbolsAreCorrect() {
    assertAll("All Suit symbols are correct",
